...
def specialSiteRegistryFile = new File(
    "data/metadata/samples/site_iri_registry.tsv"
)
if (!specialSiteRegistryFile.isFile()) {
    throw new IllegalStateException(
        "Missing stable site-IRI registry: ${specialSiteRegistryFile}"
    )
}
def registryLines = specialSiteRegistryFile.readLines()
if (registryLines.isEmpty()) {
    throw new IllegalStateException("The stable site-IRI registry is empty")
}
def registryHeader = registryLines[0].split("\t", -1)*.trim()
def registryLabelColumn = registryHeader.findIndexOf { it == "site_label" }
def registryIriColumn = registryHeader.findIndexOf { it == "site_iri" }
if (registryLabelColumn < 0 || registryIriColumn < 0) {
    throw new IllegalStateException(
        "site_iri_registry.tsv requires site_label and site_iri columns"
    )
}
def specialSiteIris = [:]
registryLines.drop(1).each { line ->
    if (!line.trim()) return
    def parts = line.split("\t", -1)
    if (
        parts.size() <= Math.max(registryLabelColumn, registryIriColumn)
        || !parts[registryLabelColumn].trim()
        || !parts[registryIriColumn].trim()
    ) {
        throw new IllegalStateException(
            "Malformed site-IRI registry row: ${line}"
        )
    }
    def label = parts[registryLabelColumn].trim()
    def iri = parts[registryIriColumn].trim()
    if (specialSiteIris.containsKey(label)) {
        throw new IllegalStateException(
            "Duplicate site label in site-IRI registry: ${label}"
        )
    }
    if (specialSiteIris.values().contains(iri)) {
        throw new IllegalStateException(
            "Duplicate site IRI in site-IRI registry: ${iri}"
        )
    }
    if (!iri.startsWith(BASE + "RAK_1")) {
        throw new IllegalStateException(
            "Site registry IRI is outside the project site namespace: ${iri}"
        )
    }
...
def unnamedByBase = finalSites.findAll { fs -> !fs.label }.groupBy {
    fs -> fs.baseName
}
unnamedByBase.keySet().sort().each { baseName ->
    def sites = unnamedByBase[baseName].sort { a, b ->
        (a.avgLon <=> b.avgLon) ?: (a.avgLat <=> b.avgLat)
    }
    sites.eachWithIndex { fs, index ->
        fs.label = sites.size() > 1
            ? "Site ${baseName} (location ${index + 1})"
            : "Site ${baseName}"
    }
}
def observedSpecialLabels = finalSites.findAll {
    fs -> !(fs.baseName ==~ /^\d+$/)
}*.label as Set
if (observedSpecialLabels != specialSiteIris.keySet() as Set) {
    throw new IllegalStateException(
        "Stable site-IRI registry mismatch. Missing from registry: " +
        "${observedSpecialLabels - specialSiteIris.keySet()}; " +
        "not observed in samplesheets: " +
        "${specialSiteIris.keySet() - observedSpecialLabels}"
    )
}
...
finalSites.each { fs ->
    def siteIriString
    if (fs.baseName ==~ /^\d+$/) {
        def numericSite = fs.baseName.toInteger()
        if (numericSite < 1 || numericSite > 60) {
            throw new IllegalStateException(
                "Numeric site ${numericSite} has no stable project IRI; " +
                "sites 61--64 are resolved through the confirmed alias ledger"
            )
        }
        siteIriString = BASE + String.format("RAK_1%06d", numericSite)
    } else {
        siteIriString = specialSiteIris[fs.label]
    }
    def siteIri = IRI.create(siteIriString)
