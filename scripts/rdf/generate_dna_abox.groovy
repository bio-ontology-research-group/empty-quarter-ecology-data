...
// SIO-canonical measurement-value linkage (see docs/MEASUREMENT_PATTERN.md):
//   value   sio:SIO_000215  quality   "is measurement value of"
...
def dnaExtractionProcessClass = df.getOWLClass(IRI.create(BASE + "RAK_0000309"))
...
// DNA-specific quality + value classes (sub PATO:0000033 / SIO:000070).
// Mint per-measurement quality + value individuals so the DNA generator
// follows the canonical SIO measurement reification pattern (see
// docs/MEASUREMENT_PATTERN.md and tests/measurement_pattern/).
def dnaConcQualityClass = df.getOWLClass(IRI.create(BASE + "RAK_0000043"))
def dnaConcValueClass   = df.getOWLClass(IRI.create(BASE + "RAK_0000044"))
...
// IRI was RAK_F000003 — collided with RAK_F<n> primer instances minted by
// generate_sra_abox.groovy. Migrated to the dedicated RAK_FN (function) prefix.
def measureFuncInd = df.getOWLNamedIndividual(IRI.create(BASE + "RAK_FN000001"))
...
int quantityCounter = 900001        // RAK_4XXXXXX — measurement values
int qualityCounter  = 900001        // RAK_5XXXXXX — qualities
...
(0..<workbook.getNumberOfSheets()).each { i ->
    ...
    sheet.drop(1).each { row ->
        ...
        // If a sample is in the SRA sheet, assume DNA exists regardless of the
        // master-sheet flag. Otherwise, treat the explicit spreadsheet sentinel
        // "NA" as absent DNA, like blank/FALSE. The only non-control NA rows are
        // the F52/F56 rhizosphere records whose raw-sample field says
        // "Not taken" and whose note says that no plant was found.
        def absentDnaStatuses = ["", "false", "na", "n/a"] as Set
        if (!sequencedSamples.contains(sampleId) &&
                absentDnaStatuses.contains(dnaStatus.toLowerCase())) return
        ...
        manager.addAxiom(ontology, df.getOWLClassAssertionAxiom(experimentClass, extProcInd))
        ...
        // Canonical SIO split pattern (matches XRF / measurements / QC / taxonomy):
        //   process  has-target → DNA extract
        //   process  has-output → value (and value is-output-of process)
        //   value    is-measurement-value-of → quality (SIO_000215)
        //   quality  has-measurement-value   → value   (SIO_000216, inverse)
        //   extract  has-attribute → quality (and quality is-attribute-of extract)
        //   value    has-numeric-literal + has-unit
        ...
        if (concVal && concVal != "" && concVal != "0" && !concVal.equalsIgnoreCase("NaN")) {
            ...
            // Quality individual — inheres in the DNA extract (RAK_5XXXXXX).
            def qualInd = df.getOWLNamedIndividual(IRI.create(BASE + "RAK_5" + String.format("%06d", qualityCounter++)))
            manager.addAxiom(ontology, df.getOWLClassAssertionAxiom(dnaConcQualityClass, qualInd))
            addLabel(qualInd.getIRI(), "DNA concentration of ${sampleId}")

            // Value individual — output of the process; carries literal + unit (RAK_4XXXXXX).
            def valInd = df.getOWLNamedIndividual(IRI.create(BASE + "RAK_4" + String.format("%06d", quantityCounter++)))
            manager.addAxiom(ontology, df.getOWLClassAssertionAxiom(dnaConcValueClass, valInd))
            addLabel(valInd.getIRI(), "DNA concentration measurement value for ${sampleId}")

            try {
                double val = Double.parseDouble(concVal)
                manager.addAxiom(ontology, df.getOWLDataPropertyAssertionAxiom(hasDNAConcentration, valInd, val))
            } catch (e) {
                manager.addAxiom(ontology, df.getOWLDataPropertyAssertionAxiom(hasDNAConcentration, valInd, concVal))
            }

            // Unit ng/uL (UO_0000275) on the value (not the quality).
            manager.addAxiom(ontology, df.getOWLObjectPropertyAssertionAxiom(hasUnit, valInd, df.getOWLNamedIndividual(IRI.create(UO + "0000275"))))
            ...
            manager.addAxiom(ontology, df.getOWLObjectPropertyAssertionAxiom(isOutputOf, valInd, measProcInd))
            manager.addAxiom(ontology, df.getOWLObjectPropertyAssertionAxiom(hasOutput, measProcInd, valInd))
            manager.addAxiom(ontology, df.getOWLObjectPropertyAssertionAxiom(isAttributeOf, qualInd, dnaInd))
            manager.addAxiom(ontology, df.getOWLObjectPropertyAssertionAxiom(hasAttribute, dnaInd, qualInd))
        }
    }
}
