...
SELECT ?processLabel ?analyte ?concentration ?error
WHERE {
  ?sample a rak:RAK_0000021 ; rdfs:label ?sampleLabel .
  FILTER(REGEX(?sampleLabel, "Site ${siteId}"))
...
while (results.hasNext()) {
    ...
    def procLabel = soln.getLiteral("processLabel").getLexicalForm()
    def match = procLabel =~ /Test ([^)]+)/
    if (match) {
        def testId = match[0][1]
